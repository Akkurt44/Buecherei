...
namespace BuechereiApp.Services
{
    public class ApiService
    {
...
#else
        private const string BaseUrl = "http://EC2_IP:8000";
#endif
        ...
        public async Task<Ausleihe> AusleiheBuch(int buchId, int mitgliedId)
        {
            var url = $"{BaseUrl}/ausleihen?buch_id={buchId}&mitglied_id={mitgliedId}";
            var response = await _http.PostAsync(url, null);
            return await response.Content.ReadFromJsonAsync<Ausleihe>();
        }

        public async Task<List<Ausleihe>> GetAusleihen()
        {
            return await _http.GetFromJsonAsync<List<Ausleihe>>($"{BaseUrl}/ausleihen");
        }

        public async Task IadeBuch(int ausleiheId)
        {
            await _http.PutAsync($"{BaseUrl}/ausleihen/{ausleiheId}/iade", null);
        }
